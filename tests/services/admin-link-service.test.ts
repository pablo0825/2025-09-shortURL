...
describe('admin-link-service', () => {
    ...
    it('should throw 409 when link is deleted in deactivate', async () => {
        ...
        await expect(deactivateAdminLinkByIdService(101)).rejects.toThrow('Link already deleted');
    ...
    it('should throw 409 when link is already disabled in deactivate', async () => {
        ...
        await expect(deactivateAdminLinkByIdService(101)).rejects.toThrow('Link already disabled');
        expect(queryMock).toHaveBeenCalledWith('ROLLBACK');
    });
    ...
    it('should throw 409 when link is already deleted in soft delete', async () => {
        ...
        await expect(deleteAdminLinkByIdService(101)).rejects.toThrow('Link already deleted');
        expect(queryMock).toHaveBeenCalledWith('ROLLBACK');
    });
