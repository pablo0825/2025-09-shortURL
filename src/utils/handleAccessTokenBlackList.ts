...
export async function handleAccessTokenBlackList (req: Request):Promise<void> {
    ...
    if (!accessToken) {
        console.warn("[api:auth/logout] Authorization header 格式錯誤");
        return;
    }
    ...
    try {
        ...
        if (typeof exp === "number") {
            ...
            console.log(`[api:auth/logout] Access Token 已加入黑名單`);
        } else {
            console.warn("[api:auth/logout] Access Token 中缺少 exp，無法加入黑名單");
        }
    } catch (err) {
        // Access Token 無效或已過期，不影響登出流程
        console.warn("[api:auth/logout] Access Token 驗證失敗，略過黑名單處理:", err);
    }
}