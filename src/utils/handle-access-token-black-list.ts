...
export async function handleAccessTokenBlackList(
    authorizationHeader?: string | null,
): Promise<void> {
    const authHeader = authorizationHeader ?? undefined;
    ...
    if (!accessToken) {
        logger.warn('Authorization header 格式錯誤');
        return;
    }
    ...
    try {
        ...
        if (typeof exp === 'number') {
            ...
            logger.info(`Access Token 已加入黑名單`);
        } else {
            logger.warn('Access Token 中缺少 exp，無法加入黑名單');
        }
    } catch (err) {
        // Access Token 無效或已過期，不影響登出流程
        logger.warn('Access Token 驗證失敗，略過黑名單處理:', err);
    }
}
