...
import type { ReplaceRolePermissionsDto } from '../../schemas/admin-schema';
...
const wrapServiceError = (context: string, error: unknown): AppError => {
    ...
    if (error instanceof Error && error.name === 'RoleVersionConflictError') {
        return new AppError(409, `[${context}] ${msg}`, error.name);
    }

    if (error instanceof Error && error.name === 'PermissionNotFoundError') {
        return new AppError(422, `[${context}] ${msg}`, error.name);
    }
...
export const manageRolePermissionsService = async (
    input: ReplaceRolePermissionsDto & { roleId: number },
    ...
    try {
        ...
        const uniquePairs = Array.from(
            new Map(
                input.permissions.map((item) => [`${item.module}|${item.type}`, item]),
            ).values(),
        );
        ...
        const currentPermissionIds: number[] = await findPermissionIdsByRoleId(
            client,
            input.roleId,
        );
        ...
        if (client) {
            try {
                ...
                throw wrapServiceError(
                    'adminRoleService.manageRolePermissions.rollback',
                    rollbackError,
                );
            }
        }
