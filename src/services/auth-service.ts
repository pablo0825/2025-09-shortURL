...
import { consumBackupCodes } from '../utils/backup-codes';
import {
  consumeBackupCode,
...
const formatServiceError = (prefix: string, err: unknown): never => {
  if (err instanceof ServiceError) {
    throw new Error(`[${prefix}] ${err.message}|${err.status}`);
  }
  const message = err instanceof Error ? err.message : String(err);
  throw new Error(`[${prefix}] ${message}|500`);
};
...
const parseRefreshTokenExpiry = (refreshToken: string): { expiresAt: Date; maxAge: number } => {
  ...
  if (typeof exp !== 'number') {
    throw new ServiceError(500, '系統錯誤，請稍後再試');
  }
...
const decodeRefreshUserId = (refreshToken: string): number => {
  ...
  if (!id || Number.isNaN(Number(id))) {
    throw new ServiceError(401, 'Refresh Token 無效，請重新登入');
  }
  return Number(id);
};

export const registerService = async (input: RegisterInput): Promise<RegisterResult> => {
  try {
    ...
    const created = await withTransaction(async (client) => {
      ...
      if (!roleId) {
        throw new ServiceError(409, '角色 user 尚未在role table建立');
      }
    ...
    if (pgErr?.code === '23505') {
      if (pgErr.constraint === 'users_email_uk') {
        throw new ServiceError(409, 'Email 已經被註冊過!');
      }
      if (pgErr.constraint === 'users_nickname_uk') {
        throw new ServiceError(409, '暱稱已經被使用!');
      }
      throw new ServiceError(409, '使用者資料已存在，請勿重複註冊!');
    }
    return formatServiceError('authService.register', err);
  }
};
...
export const loginService = async (
  ...
  try {
    ...
    return formatServiceError('authService.login', err);
  }
};
...
export const login2faService = async (
  ...
  try {
    ...
    if (!params.ok) {
      throw new ServiceError(401, '2FA token 無效或已過期');
    }

    const raw = await cacheGetDel(`2fa:login:${params.jti}`);
    if (!raw || Number(raw) !== params.userId) {
      throw new ServiceError(401, '2FA token 已使用或已過期');
    }
    ...
    if (!user) {
      throw new ServiceError(404, '使用者不存在或資料異常');
    }
    if (!user.twofa_enabled) {
      throw new ServiceError(400, '此帳號未啟用 2FA');
    }
    if (!user.role_type) {
      throw new ServiceError(500, '系統錯誤，請稍後再試');
    }
    ...
    if (input.method === 'totp') {
      ...
      if (!encrypted || !iv || !authTag) {
        throw new ServiceError(400, '2FA 資料不完整，請重新設定');
      }

      const secret = decrypt(encrypted, iv, authTag);
      const valid = verifyTotpCode(input.code, secret);
      if (!valid) {
        throw new ServiceError(400, '驗證碼錯誤');
      }
    } else {
      const version = user.twofa_backup_codes_version;
      if (!version || version <= 0) {
        throw new ServiceError(400, '尚未設定備用碼');
      }

      const hashes = await findAvailableBackupCodeHashes(userId, version);
      if (!hashes.length) {
        throw new ServiceError(400, '無有效的備用碼');
      }

      const matched = await consumBackupCodes(input.code, hashes);
    ...
    await withTransaction(async (client) => {
      ...
      if (usedBackupCodeHash) {
        ...
        if (!consumed) {
          throw new ServiceError(400, '備用碼已使用或無效');
        }
      }
    ...
    return formatServiceError('authService.login2fa', err);
  }
};
...
export const refreshService = async (
  ...
  try {
    ...
    return await withTransaction(async (client) => {
      ...
      if (!user) {
        throw new ServiceError(401, '帳戶不存在或已被停用，請重新登入');
      }
      ...
      if (!sessionId) {
        throw new ServiceError(401, '更新失敗，請重新登入');
      }
    ...
    return formatServiceError('authService.refresh', err);
  }
};
...
export const logoutService = async (refreshToken: string): Promise<LogoutResult> => {
  try {
    ...
    return formatServiceError('authService.logout', err);
  }
};
...
export const forgotPasswordService = async (
  ...
  try {
    ...
    const count = await countRecentUserAction(
      user.id,
      UserLogActionEnum.FORGOT_PASSWORD,
      5,
    );
    if (count > 0) {
      throw new ServiceError(429, '請稍後再試，您已在 5 分鐘內請求過密碼重設');
    }
    ...
    await writeUserLogToDB(user.id, UserLogActionEnum.FORGOT_PASSWORD, {
      detail: '使用者請求重設密碼',
    ...
    return formatServiceError('authService.forgotPassword', err);
  }
};
...
export const resetPasswordService = async (
  ...
  try {
    ...
    return formatServiceError('authService.resetPassword', err);
  }
};
