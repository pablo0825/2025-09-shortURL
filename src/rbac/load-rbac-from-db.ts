...
async function _loadRbacFromDb() {
    ...
    const permissions = roles.rows.map((role: { id: number; type: string }) => {
        return pool.query<{ module: string; type: string }>(
            'SELECT p.module, p.type FROM permissions p JOIN role_permissions rp ON p.id = rp.permissions_id WHERE rp.role_id = $1',
            [role.id],
        );
    });
    ...
    const redisWrites = roles.rows.map(
        async (role: { id: number; type: string }, index: number) => {
            const redisKey = buildCacheKey('role', `${role.type}:permissions`);

            // 把module, type等欄位組合
            const permission = permissionResults[index].rows.map(
                (p: { module: string; type: string }) => {
                    // 如：read_profile:user
