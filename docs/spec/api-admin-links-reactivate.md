# PATCH /admin/links/reactivate

## Purpose
作為一位管理員，我希望用 linkId 來批次重新啟用 link，這樣我才可以快速恢復被停用的 link。
...
```json
{
  ...
  "data": {
    "succeeded": [
      {
        "id": 101,
        "before": { "isActive": false, "status": "disabled" },
...
- WHEN ids 中部分 id 對應的 link 狀態為 "active"，系統應將該筆加入 failed，reason: "連結已是啟用狀態，無需重新啟用"
- WHEN ids 中部分 id 對應的 link 狀態為 "deleted"，系統應將該筆加入 failed，reason: "連結已刪除，請使用 restore"
- WHEN ids 中部分 id 對應的 link 狀態為 "expired"，系統應將該筆加入 failed，reason: "連結已過期，無法重新啟用"
- WHEN 操作完成後存在至少一筆 succeeded 及一筆 failed，系統應回傳 207
- WHEN 所有 id 皆成功，系統應回傳 200
- WHEN 所有 id 皆失敗，系統應回傳 422，並說明 error message: "所有連結皆無法重新啟用"
