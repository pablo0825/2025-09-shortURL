...
嚴格遵循以下順序，不得跨層呼叫。如有特殊架構需求需要例外，須在本文件中明確說明。
...
- 執行任何非 trivial 的任務前，先說明理解到的需求與執行計畫，再動手。
- 遇到需要人類決策的情況（如架構選擇、不確定需求、操作前需確認的項目），採用**一問一答**模式：一次只問一個問題，並提供選項與每個選項的理由，等待確認後再繼續。
- 回答問題或分析問題時，依複雜度調整說明深度：簡單任務給出精簡摘要，複雜或涉及架構決策的任務展示思考過程。
